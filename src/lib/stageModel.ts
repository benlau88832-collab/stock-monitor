// ============================================================
// 主线阶段模型（v9.27 · P0-3；v9.129.0 一致性收敛——概念边界声明）
// v9.129.0：**本模块 = 题材维度"主线阶段"**（单条主线的生命周期：启动期→发酵期→…），
//   与市场维度"情绪周期"（emotionStage.deriveStage 六词，与认知层同构）是两个不同概念，
//   不再互相映射（原 emotionToStage 已删——曾造成"冰点→退潮期"的词表混用）。
// 词表：五态 + 观察中（信号不一致时的兜底态），仅用于主线卡/题材诊断。
...
// ============================================================
// v9.129.0（一致性收敛）：入口 3（emotionToStage 市场→主线映射）已删除——
//   情绪周期（市场维度）以 emotionStage.deriveStage 六词为准，不再映射为主线词表。
// ============================================================
