...
describe("v9.107.0 fallbackAnswer（规则兜底，任何问题都有回答）", () => {
  it("主线类 → 列主线Top3 + 闸门，带统一前缀", () => {
    const r = fallbackAnswer(SNAPSHOT, "今日前三的主线和龙头标的是什么");
    expect(r.startsWith("⚠ 规则版（AI 暂不可用")).toBe(true);
    expect(r).toContain("医药(强度80·发酵期·裁决观望·龙头:百花医药/开开实业/哈药)");
    expect(r).toContain("次日闸门");
  });

  it("个股类（代码/名字）→ 快照中该股相关数据，无则明说", () => {
    const r = fallbackAnswer(SNAPSHOT, "百花医药为什么涨停");
    expect(r).toContain("百花医药");
    expect(r).toContain("青蒿素");
    const r2 = fallbackAnswer(SNAPSHOT, "贵州茅台走势如何");
    expect(r2).toContain("本地无 贵州茅台 的盘口数据");
  });

  it("消息类 → 本地最近2日消息摘要（政策优先）", () => {
    const r = fallbackAnswer(SNAPSHOT, "周末有什么重要消息");
    expect(r).toContain("国常会部署新型消费");
    expect(r).toContain("证监会完善退市制度");
  });

  it("情绪/行情类 → 市场行 + 涨停板块分布", () => {
    const r = fallbackAnswer(SNAPSHOT, "今天情绪如何");
    expect(r).toContain("情绪68");
    expect(r).toContain("涨停板块分布");
  });

  it("其他问题 → 快照通用摘要（非空白）", () => {
    const r = fallbackAnswer(SNAPSHOT, "随便聊聊");
    expect(r).toContain("【本地数据摘要】");
    expect(r.length).toBeGreaterThan(20);
  });

  it("reason 透传（empty content/超时分类）", () => {
    const r = fallbackAnswer(SNAPSHOT, "今天情绪如何", "empty content/调用失败");
...
describe("v9.107.0 brainContextToText（上下文全量注入）", () => {
  ...
  it("主线 Top3 含强度/趋势/裁决/龙头前3 + 数据截至时间戳", () => {
