...
export async function fetchMarketBreadth(): Promise<MarketBreadth> {
  ...
  const url = `${PUSH2}/clist/get?pn=1&pz=6000&po=1&np=1&fltt=2&invt=2&fid=f3&fs=${fs}&fields=f2,f3,f12`;
  const json = await emGet(url, 12000);
...
export async function fetchBoardFundFlow(
  ...
  const fields = "f12,f14,f3,f62,f164,f165,f174,f175,f184";
...
export async function fetchBoardConstituents(boardCode: string, limit = 10): Promise<FundFlowRow[]> {
  const fields = "f12,f14,f2,f3,f62,f66,f78,f84,f184,f8,f9,f10";
...
export async function fetchAnnouncementRisk(code: string) {
  try {
    const url = `${ANN_API}?sr=-1&page_size=40&page_index=1&ann_type=A&client_source=web&stock_list=${code}&f_node=0&s_node=0`;
    const json = await emGet(url, 8000);
    ...
    return { available: false, reduction: [], regulatory: [] };
  }
}

export async function fetchGlobalNews(size = 12) {
  try {
