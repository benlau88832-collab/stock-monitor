import { boardRealUrl, fundFlowUrl, marketBreadthUrl } from "../lib/realLinks";
  ...
  const items = [
    {
      ...
      link: boardRealUrl("", "concept"),
    },
    {
      title: "2. 盲目跟风（无资金验证）",
      desc: "仅看概念热度或涨幅，不验证资金结构。当主力持续流出而散户净流入时，结构极不健康。",
      realDataSource: "东方财富分档资金流（f62主力、f84小单、f164/165/174/175连续性）",
      avoidTip: "任何操作前先看「资金结构速览」：主力净流出 + 散户净流入 = 一票否决。",
      link: fundFlowUrl(),
    },
    {
      title: "3. 忽视风险信号（质押/减持/监管）",
      desc: "高股权质押比例（≥50%）存在平仓风险；大股东减持意味着筹码松动；监管问询直接打击信心。",
      realDataSource: "东方财富数据中心 RPT_CSDC_LIST（质押）、公告扫描（减持/监管关键词）",
      avoidTip: "风险雷达自动扫描：质押≥30%标记、减持/监管公告标红。",
      link: "https://data.eastmoney.com/gpzy/",
    },
    {
      title: "4. 过度交易（高换手率拥挤）",
    ...
    },
    {
      ...
      desc: "主力持续净流出（今日+5日+10日均为负）+ 散户净流入 = 典型「主力出、散户进」危险结构。",
      realDataSource: "东方财富资金结构接口（f62/f66/f72/f78/f84/f164/f165/f174/f175）",
      avoidTip: "市场概览顶部自动显示「一票否决警报」：结构危险时不建议加仓。",
      link: fundFlowUrl(),
    },
  ];
    ...
    <section className="rounded-xl border border-rose-500/30 bg-gradient-to-br from-rose-950/20 to-rose-900/10 p-5">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400">基于东方财富真实数据接口的风险信号识别。所有数据真实可查。</p>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-lg bg-black/30 border border-white/5 px-4 py-3 text-[11px] text-slate-500 leading-relaxed">
        ...
        本终端所有数据均来自东方财富公开接口真实抓取。任何结论仅供参考，不构成投资建议。
        A股市场高波动、高博弈、低披露，散户最容易在「结构危险时盲目操作」时踩坑——
        本终端通过「资金结构优先、风险信号优先、一票否决规则、真实数据可验证」四重机制帮助你避开常见陷阱。
      </div>
    </section>
  );
}
