...
// ============== 情绪温度计（升级版） ==============
function SentimentGauge({ value, label, factors, yesterday }: {
...
export default function MarketOverview({ data, loading }: { data: OverviewData | null; loading: boolean }) {
  ...
  const { indices, breadth, sentiment, sentimentLabel, sentimentFactors, sentimentYesterday, limitPool, turnoverAmount, turnoverYesterday, turnoverAvg5d } = data;
  ...
  return (
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px]">
      ...
      {/* 右侧：情绪温度计（升级版） */}
      <SentimentGauge value={sentiment} label={sentimentLabel} factors={sentimentFactors} yesterday={sentimentYesterday} />
    </section>
  );
}
