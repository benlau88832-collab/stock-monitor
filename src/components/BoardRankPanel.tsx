    ...
    <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-4">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {sortedList.filter(b => { const k = classifyBoard(b.name); return k === "industry" || k === "theme"; }).map(b => {
              const kind = classifyBoard(b.name);
              return (
            ...
            {sortedList.filter(b => { const k = classifyBoard(b.name); return k === "industry" || k === "theme"; }).length === 0 && (
      ...
      {/* 风格标签折叠组 */}
      <StyleBoardsGroup boards={sortedList.filter(b => { const k = classifyBoard(b.name); return k === "style" || k === "region"; })} />
...
// ============== 风格标签折叠组 ==============
function StyleBoardsGroup({ boards }: { boards: BoardRankItem[] }) {
  const [open, setOpen] = useState(false);
  if (boards.length === 0) return null;
  return (
    <div className="border-t border-white/10 pt-2 mt-2">
      <button onClick={() => setOpen(v => !v)} className="text-[11px] text-slate-500 hover:text-slate-400">
        {open ? "▲ 收起" : "▼"} 市场结构观察（机构行为与风格标签）· {boards.length}项
      </button>
      {open && (
        <div className="mt-1">
          <div className="text-[11px] text-slate-600 mb-1">主体/风格标签仅作波段情报参考，不作题材推荐</div>
          <div className="space-y-0.5">
            {boards.map(b => (
              <div key={b.code} className="flex items-center justify-between text-[11px] py-0.5 hover:bg-white/5">
                <span className="text-slate-400">{b.name}</span>
                <div className="flex gap-2">
                  <span className={pctColor(b.mainNet)}>{fmtMoney(b.mainNet)}</span>
                  <span className={pctColor(b.pct)}>{fmtPct(b.pct)}</span>
                </div>
              </div>
            ))}
          </div>
