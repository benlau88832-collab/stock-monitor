...
export default function Dashboard() {
  ...
  const [globalSignals, setGlobalSignals] = useState<any>(null);
  ...
  const refreshAll = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [ov, fs, rr, ml, nw, dp, gs] = await Promise.all([
        ...
        safeJson("/api/market/global-signals"),
      ]);
      setOverview(ov);
      setFundStructure(fs);
      setRiskRadar(rr);
      ...
      <main className="mx-auto max-w-[1500px] space-y-6 px-4 py-6">
        {active === "market" && (
          <>
            ...
            <GlobalSignals data={globalSignals} loading={loading} />
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              <NewsPanel data={news} loading={loading} />
            </div>
          </>
        )}
