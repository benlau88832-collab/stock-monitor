...
function formatMainNet(v: number | null | undefined): string {
  ...
  const a = Math.abs(n);
  const sign = n < 0 ? "-" : "";
  return `${sign}${a >= 1e8 ? `${(a / 1e8).toFixed(2)}亿` : `${(a / 1e4).toFixed(1)}万`}`;
}
...
// v9.144.0：市场盘面优先取 /api/cognition，避免 localStorage 空值与页面温度矛盾。
async function buildSupervisorPrompt(question: string): Promise<{ system: string; user: string }> {
  ...
  try {
    ...
    dataParts.push(`【市场盘面】情绪温度计：${s?.score ?? "暂缺"}分 · 阶段=${s?.stage ?? "暂缺"} · 溢价=${s?.premium ?? "暂缺"}`);
    dataParts.push(`【认知层主线】${cog?.mainline?.value?.primaryTheme ?? "暂缺"} 强度${cog?.mainline?.value?.strength ?? "?"} · ${cog?.mainline?.value?.hotspotRotation ?? "?"}`);
  } catch {
    dataParts.push("【市场盘面】情绪温度计：暂缺");
  }
  ...
  try {
    ...
    const fundStr = mainFund.status === "fulfilled"
      ? `主力资金：今日=${formatMainNet(mainFund.value.mainNet)} 5日=${formatMainNet(mainFund.value.mainNet5d)} 10日=${formatMainNet(mainFund.value.mainNet10d)}`
      : "主力资金：暂缺";
  ...
  try {
    ...
    if (limitPool && limitPool.rawZTPool && limitPool.rawZTPool.length > 0) {
      const details = limitPool.rawZTPool.slice(0, 15).map((s: any) => `${s.name || s.n}(${s.c})`).join("/");
  ...
  try {
    ...
    if (marketAnns.length > 0) {
      const recentAnns = marketAnns.slice(0, 10).map(a => `[${a.stockCode}]${a.stockName}:${a.title}`).join("; ");
  ...
  try {
    ...
    if (fastNews.length > 0) {
      const recentNews = fastNews.slice(0, 10).map(n => `${n.title}(${n.time.slice(11, 16)})`).join("; ");
  ...
  try {
    ...
    if (boardRank?.inflow?.length > 0) {
      const topInflow = boardRank.inflow.map(b => `${b.name}:净流入${formatMainNet(b.mainNet)}`).join("; ");
      dataParts.push(`【资金流入Top】${topInflow}`);
    }
    if (boardRank?.outflow?.length > 0) {
      const topOutflow = boardRank.outflow.map(b => `${b.name}:净流出${formatMainNet(b.mainNet)}`).join("; ");
  ...
  // v9.75（深化）：注入信号账本 T+1/T+5 命中率 —— 让督导能引用历史决策效果
  try {
