...
## v9.30.1 — 修复"主力净流出 0 行业"根因：行业板块拉取截断 (2026-08-06)

### 用户反馈
v9.30 部署后资金走势图"主力净流出（0 行业）"仍存在。

### 根因（实测定位）
- 东财 clist/get 的 `po=1` 是 **f62 降序**；`fetchBoardFundFlow` 用 `po=1&pz=limit(30)`
  → 只拿到 **f62 最大的前 30 个行业（全是流入）**，流出行业被接口截断，根本没进结果集
- 实测全量 100 个行业：90 个下跌、大量 f62 为负（银行 -4.37亿/食品饮料 -16.36亿）
  —— 数据源有流出，是拉取逻辑丢的

### 修复
- `fetchBoardFundFlow` 加 `opts.all`：all=true 时 `pz=500` 拉全量 → 本地按 mainNet 降序返回全部
- App.tsx 行业数据源改 `fetchBoardFundFlow("industry", 30, { all: true })`
- 资金走势图红绿双榜数据完整（流入榜 + 流出榜同时有值）
