...
> 针对"加载卡慢/模块加载不出来"。核心：**LLM 主线归类不再阻塞作战卡首次渲染** + **AI 并发信号量**（响应 Agnes 并发限流）。

### 🔴 性能（加载卡慢根因修复）
- **渐进式主线渲染**：原 `classifyStocksToMainlines`(LLM) 在 refreshAll 里同步 await，作战卡/裁决卡/选股清单要等 LLM 完成才出现（Agnes 慢/并发限流时卡 10-30s）。现改为：`skipLLM` hybk 硬分类（~1-3s，无 LLM）**先渲染** → LLM 软语义归类**异步升级**（`battleSeq` 轮次护栏防慢 LLM 覆盖新数据）。首屏作战卡从"等 LLM"变成"秒出规则版 + 1-5s 后 LLM 精化"
- **AI 并发限流器**：`ai.ts` 新增信号量（≤3 在飞）——主线归类/Agent/快讯/个股研判/精排并发高峰时排队，不再同时打到 Agnes 并发上限导致超时降级
- **LLM 主线精排 20 分钟节流**（v9.77 A8-04）：精排不再每次 refreshAll 烧 analysis 桶

### ✅ 工程可用性验证（本批次）
- 新增 9 项回归测试（封单红档/核按钮分档/仓位聚合截断/卡位降级）—— 221 全绿
- 因子"样本不足<5"不再误标"失效"（A9-7，原启动日志 9/11 假失效压低 AI 置信）
- express.json 2mb→10mb（消除 localStorage 全量迁移 PayloadTooLargeError 静默丢数据）
- 涨停池 degraded 周末中性措辞（"数据来自 08-07"非"接口异常"）
- 仓位聚合截断改纯截断（不角色降权，避免 5% 保底仓打折致清单塌缩）
...
> 核心：修好"strengthCompleteness 永远 0.25"级联根因（主线不再永远"无可交易"），五套分类体系统一到 classifyStock，因子样本<30 不再误杀封锁。

### 🔴 P0 级联解锁 + 概念统一 + 因子门控（V11-1/2/3/8/9/11/12）
- **V11-1** mainlineScore：completeness 结构性缺失不计入（晋级率基础 60% + 进阶可选 40%），**下限 0.5**——"永远无可交易"级联根因修复
- **V11-2** FiveQBar 龙头：text-lg font-black + 主线名兜底 + 板数/理由
- **V11-3** runAgent：数据未显著变化（分差<10/涨停<3）复用上次裁决 + 裁决变化提示（"上次=X"）
- **V11-8** stockToMainline：`foldLLMResult` 统一折叠（光通信/CPO→通信）+ 事件主题 ⚡ 前缀 + groups 合并；**⚡ 前缀资金匹配剥除（自验证补漏）**
- **V11-9** decisionBus/aiAgent：因子 penalty 加 `(samples??0)>=30` 硬门控（注释承诺代码遗漏修复，样本不足不再误杀）
- **V11-11** 新建 `classifyStock.ts` 全站唯一分类器（F10 投票→hybk→申万），集成 themeLadder 分组
- **V11-12** LLM prompt 硬约束：主线名 MUST 从 24 大类选（动态注入），禁事件主题，兜底"其他"

### 🟠 P1 驾驶舱 + 图表 + AI 激活 + 分类校准（V11-4/5/6/10/13）
- **V11-4** GlobalSignals 移出驾驶舱（fundline"🌐外围信号"已有，删重复渲染）
- **V11-5** EventClassifyPanel 移回驾驶舱（裁决→选股→事件）+ 可点击跳消息面 + 方向↑↓ + 力度★ + 受益数
- **V11-6** estimateMissingFields 工具（换手/晋级/10日资金推断）+ Agent system 规则 + 裁决"基于部分推断"标注
- **V11-10** 行业资金流向图：伪分时曲线 → **横向条形图**（流入红右伸/流出绿左伸，不错位）
- **V11-13** conceptGroups 校准：华为昇腾→算力、半导体设备并入芯片、宠物经济→大消费

### 🟡 P2（V11-7）
- **V11-7** StockPickList 强度不足/闸门收紧时标题旁明示"候选参考（非正式推荐）"

### 遵循 CLAUDE.md 第十四节自验证协议
- 每项执行：门1 tsc 0 / 门2 build 成功 / 门3 test 全绿（最终 **170 全绿**，+10 新增用例）
- 阶段2 跨文件一致性 grep：⚡资金匹配剥前缀（补漏）、GlobalSignals 重复渲染定位、EventClassifyPanel 悬空 import 清理（tsc 抓到）
- 阶段3 防漏清单逐条打勾
